(() => {
  ...
  const V9_AFFECTED_SECTIONS = new Set(['5','12','14','21','22','23','25_2','30']);
  const isMarginaliaId = id => canon(id) === V9_MARGINALIA_ID;
  const defaultRankedRows = () => (Q['כל_הזוגות_מדורגים'] || []).filter(r => !isMarginaliaId(r.comm_a) && !isMarginaliaId(r.comm_b));
  const defaultDecompRows = () => (Q['פירוק_התפזרות'] || []).filter(r => !isMarginaliaId(r.a) && !isMarginaliaId(r.b));
  const analysisRankedRows = () => state.includeMarginalia ? (Q['כל_הזוגות_מדורגים'] || []) : defaultRankedRows();
  const analysisDecompRows = () => state.includeMarginalia ? (Q['פירוק_התפזרות'] || []) : defaultDecompRows();
  ...
  const coClusterMap = new Map((Q['קו_אשכול_בין_חיבורים'] || []).filter(r => Number.isFinite(num(r.co_cluster_rate))).map(r => [pairKey(r.comm_a, r.comm_b), r]));
  const legacyCoClusterMap = new Map((Q['קו_אשכול_בין_חיבורים'] || []).map(r => [pairKey(r.comm_a, r.comm_b), r]));
  ...
  function renderQuantCards() {
    ...
    const key=pairKey(state.a,state.b),cc = coClusterMap.get(key), legacyCc=legacyCoClusterMap.get(key);
    const oldA = oldAlign(state.section,state.a);
    const oldB = oldAlign(state.section,state.b);
    const ccPending=!cc&&legacyCc;
    const zPending=p?.v9_status==='requires_pipeline_rerun'||p?.within_section_z===''||p?.within_section_z==null;
    const cards = [
      ...
      qCard('קו־אשכול', cc?.co_cluster_rate, cc ? `${cc.shared_sections} סעיפים משותפים` : ccPending?`⟦תיקון V9⟧ מושהה; V8 ההיסטורי: ${fmt(legacyCc.co_cluster_rate)}`:'אין נתון רוחבי לזוג', 'נתונים חדשים · כימות'),
      qCard('חריגות בתוך הסעיף', zPending?null:p?.within_section_z, zPending?`⟦תיקון V9⟧ דורש rerun; V8: ${fmt(p?.legacy_within_section_z)}`:'within-section z', 'נתונים חדשים · כימות'),
    ...
    root.innerHTML=`
      ...
      <div class="grid-2">
        ...
        <section class="panel layer-quant"><div class="panel-head"><div><h3>קרבה לרמב״ן — פירוק מדדים</h3><p>מדדי הקרבה החדשים מוצגים לצד align, ללא ערבוב בין שיטות החישוב.</p></div><span class="badge badge-quant">נתונים חדשים · כימות</span></div>
          ${close?`<div class="grid-2">${qCard('D1 תוכן',close.content_D1cov,'חפיפת תוכן','נתונים חדשים · כימות')}${qCard('D5 סדר',close.order_D5,'סדר רעיונות','נתונים חדשים · כימות')}${qCard('קו־אשכול',close.co_cluster,'⟦תיקון V9⟧ מושהה עד rerun','נתונים חדשים · כימות')}${qCard('רגיסטר',close.register_sim,'דמיון רגיסטר','נתונים חדשים · כימות')}</div>`:'<div class="notice">אין שורת קרבה לרמב״ן לחיבור זה בנתונים החדשים.</div>'}
        </section>
      </div>
  ...
  function renderSharedConceptsTable(section){
    ...
    return `<div class="table-wrap" style="max-height:430px"><table class="data-table"><thead><tr><th>מושג גלובלי</th><th>תוויות</th><th>גרפים עם המושג</th><th>גרפים בקטע</th><th>שיעור</th><th>מצב V9</th></tr></thead><tbody>${rows.map(r=>{const pending=r.v9_status==='requires_pipeline_rerun';const den=num(r.n_graphs_in_section),n=num(r.n_graphs_with),rate=(!pending&&Number.isFinite(n)&&Number.isFinite(den)&&den)?n/den:null;return `<tr><td class="ltr">${esc(r.global_key)}</td><td>${esc(r.labels)}</td><td class="num">${pending?`${fmt(r.n_graphs_with,0)}*`:fmt(r.n_graphs_with,0)}</td><td class="num">${fmt(r.n_graphs_in_section,0)}</td><td class="num">${pending?'—':fmt(rate)}</td><td>${pending?'<span class="badge v9-correction">⟦תיקון V9⟧ דורש חישוב מחדש</span>':'תקין'}</td></tr>`}).join('')}</tbody></table>${rows.some(r=>r.v9_status==='requires_pipeline_rerun')?'<div class="notice v9-correction tiny"><strong>* ⟦תיקון V9⟧</strong> בקטעים שבהם הייתה שכבת השוליים, המכנה תוקן אך אין בחבילה את חומר הביניים שמאפשר לדעת האם שכבת השוליים נכללה במונה של כל מושג. לכן שיעור השיתוף אינו מוצג עד להרצת הפייפליין המקורי.</div>':''}</div>`;
  }
  ...
  async function renderClusters(){
    ...
    root.innerHTML=`
      <section class="panel cluster-hero">
        <div class="panel-head"><div><div class="eyebrow">V9 · Research questions as visual evidence</div><h2>אשכולות, קרבה ופיזור של הפרשנים</h2><p>עמוד זה מרכז את השאלות המחקריות של חבילת הנתונים החדשים ומציג אותן בשתי רמות: <strong>מקור בודד (§)</strong> ו<strong>הקורפוס כולו</strong>. כל תצוגה מסומנת לפי שכבת הנתונים שממנה היא נגזרת.</p></div><label class="small-btn" title="ברירת המחדל מוציאה את שכבת השוליים מאוכלוסיית הפרשנים"><input id="includeMarginaliaCluster" type="checkbox" ${state.includeMarginalia?'checked':''}> כלול שכבות שוליים</label></div>
        <div class="research-question-grid">
          ...
          ${researchQuestionCard('q-ramban','4','האם יש מרכז?','קרבה לרמב״ן: שלושה מדדים ישירים ב־V9; קו־אשכול מושהה עד rerun.')}
      ...
      <section id="q-distance" class="panel layer-quant cluster-pane" data-cluster-pane="distance">
        <div class="panel-head"><div><span class="badge badge-quant">חבילת הנתונים החדשים · ${analysisRankedRows().length.toLocaleString('he-IL')} זוגות־קטע בברירת־המחדל</span><h2>מי קרוב ומי רחוק?</h2><p>התפלגות מלאה של ציוני הדמיון. הציון הגולמי משמש לסדר, ואילו <em>bits</em> מראה כמה ראיה יש ביחס לזוגות הביקורת.</p></div><button class="small-btn" data-open-report="01_ממצאים">פתח את הדוח המלא</button></div>
        <div class="report-kpi-row"><div><strong>45</strong><span>שילובי שיטה × מדד שנבדקו</span></div><div><strong>29</strong><span>צירים בלתי־תלויים לפי הדוח</span></div><div><strong>600</strong><span>זוגות ביקורת</span></div><div><strong>97.0%</strong><span>האחוזון של raw=0.50 ב־V9</span></div></div><div class="tiny muted" style="margin-bottom:12px">45→29 ו־600 הם נתוני סיכום מן הדוח; חומר הגלם של בדיקות התלות וזוגות הביקורת אינו נכלל ב־CSV ולכן אינו מחושב מחדש כאן.</div><div class="grid-2"><div class="chart-box"><h3>D1 / raw — כל הזוגות</h3><div id="rawHistogram"></div></div><div class="chart-box"><h3>bits — כל הזוגות</h3><div id="bitsHistogram"></div></div></div>
        ${renderDistanceSummary()}
        <div class="details-grid"><details class="data-details"><summary>${analysisRankedRows().filter(r=>num(r.raw)>=.5).length} הזוגות שמעל raw=0.50</summary>${renderThresholdPairs()}</details><details class="data-details"><summary>הקצוות: 25 הקרובים + 25 הרחוקים</summary>${renderDistanceExtremes()}</details></div><details class="data-details"><summary>פתח את מלוא ${analysisRankedRows().length.toLocaleString('he-IL')} זוגות־הקטע בברירת־המחדל</summary><div class="lazy-table-placeholder" data-lazy-table="all-ranked">הטבלה תיטען בעת הפתיחה.</div></details>
      </section>
      ...
      <section id="q-cluster" class="panel cluster-workbench cluster-pane" data-cluster-pane="cluster">
        ...
        <div class="report-finding-strip"><div><strong>⟦תיקון V9⟧</strong> המספרים 13/36, 3 סעיפים וחציון קו־אשכול 0.32 הם תוצרי הפייפליין הישן ותלויים באוכלוסייה שבה הערות השוליים נספרו כעד עצמאי. הם נשמרים בדוח ההיסטורי בלבד ומחכים להרצה מחדש.</div><div><strong>מה כן נשמר:</strong> מדידות הזוג הישירות. במיוחד, הזוג שסומן בעבר 107↔114 הוא למעשה <strong>ותיקן 107 ↔ ותיקן 214 (הטקסט הראשי)</strong>.</div></div>
        ...
        <div class="cluster-level-block">
          ...
          <div style="margin-top:16px"><h4>אותם נתונים כמטריצה מלאה</h4><div id="clusterGlobalMatrix"></div></div><div class="cluster-dendro-grid"><div class="chart-box"><h4>דנדרוגרמה — כל הפרשנים יחד</h4><p class="tiny muted">Average-linkage על המדד שנבחר. בניגוד לרשת הסף, אף פרשן אינו נעלם גם כאשר הוא רחוק מן האחרים.</p><div id="globalDendrogram"></div></div><div class="chart-box"><h4>⟦תיקון V9⟧ כתב יד אחד, שתי שכבות</h4><p class="tiny muted">מטריצת mean raw ישיר: דעת החכם 107, הטקסט הראשי של 214, הערות השוליים של 214 והפירוש הלא־נודע מאסקוריאל. כך רואים את הקרבה של שני עדי הטקסט ואת הזיקה הנפרדת של השוליים.</p><div id="anomalyHeatmap"></div></div></div>
      ...
      <section id="q-dispersion" class="panel layer-quant cluster-pane" data-cluster-pane="dispersion">
        ...
        <details class="data-details"><summary>פתח את מלוא ${analysisDecompRows().length.toLocaleString('he-IL')} רשומות ההתפזרות</summary><div class="lazy-table-placeholder" data-lazy-table="decomp-full">הטבלה תיטען בעת הפתיחה.</div></details>
      </section>

      <section id="q-ramban" class="panel layer-quant cluster-pane" data-cluster-pane="ramban">
        <div class="panel-head"><div><h2>קרבה לרמב״ן — לא דומיננטיות</h2><p>⟦תיקון V9⟧ D1, D5 ורגיסטר נשמרו כמדדים ישירים; co-cluster מושהה עד הרצה מחדש. mean_rank חושב מחדש משלושת המדדים הישירים בלבד.</p></div><span class="badge badge-quant">קרבה_לרמבן.csv · 12 חיבורים</span></div>
    ...
    root.querySelectorAll('[data-cluster-scroll]').forEach(b=>b.addEventListener('click',()=>{const v=(b.dataset.clusterScroll||'').replace('#q-','');state.clusterView=v==='cluster'?'cluster':v;applyClusterWorkspace(root);requestAnimationFrame(()=>renderClusterPaneViews(root,state.clusterView));root.querySelector('.cluster-subnav')?.scrollIntoView({behavior:'smooth',block:'start'});}));
      root.querySelector('#includeMarginaliaCluster')?.addEventListener('change',e=>{state.includeMarginalia=e.target.checked;renderClusters()});
  ...
  function renderClusterPaneViews(root,view){
    ...
    if(v==='distance'){
      renderHistogram(root.querySelector('#rawHistogram'),analysisRankedRows().map(r=>num(r.raw)).filter(Number.isFinite),{min:0,max:1,bins:20,threshold:.5,label:'raw'});
      const bits=analysisRankedRows().map(r=>num(r.bits)).filter(Number.isFinite);renderHistogram(root.querySelector('#bitsHistogram'),bits,{bins:22,label:'bits'});
    } else if(v==='cluster'){
  ...
  function clusterGlobalMetricOptions(sel){return [['co_cluster','קו־אשכול · V8 היסטורי / מושהה ב־V9'],['mean_raw','דמיון תוכן ממוצע · חדש'],['mean_locus','חפיפת מוקדים ממוצעת · חדש'],['mean_reading','הסכמת קריאה ממוצעת · חדש'],['signature','חתימת קורפוס · נתונים ישנים']].map(([v,l])=>`<option value="${v}" ${v===sel?'selected':''} ${v==='co_cluster'?'disabled':''}>${l}</option>`).join('')}
  ...
  function clusterIdsGlobal(){return [...new Set(quantCommentators.filter(c=>c!=='RAMBAN'&&(state.includeMarginalia||!isMarginaliaId(c))))].sort((a,b)=>displayName(a).localeCompare(displayName(b),'he'))}
  function clusterIdsLocal(section,metric){const ids=new Set();const add=id=>{id=canon(id);if(id!=='RAMBAN'&&(state.includeMarginalia||!isMarginaliaId(id)))ids.add(id)};if(metric==='old_similarity'){const sec=window.RAMBAN_V2_SECTIONS[String(section)];(sec?.comparisonsBetweenCommentators||[]).forEach(r=>{add(r.source);add(r.target)})}else{const rows=metric==='raw'?(bySectionPairs.get(String(section))||[]):(bySectionDecomp.get(String(section))||[]);for(const r of rows){add(r.comm_a||r.a);add(r.comm_b||r.b)}}return [...ids].sort((a,b)=>displayName(a).localeCompare(displayName(b),'he'))}
  ...
  function renderOldLocalClusters(section){const sec=window.RAMBAN_V2_SECTIONS[String(section)];const useLegacy=state.includeMarginalia&&Array.isArray(sec?.localClustersLegacy);const cls=(useLegacy?sec?.localClustersLegacy:sec?.localClusters)||[];if(!cls.length)return `<div class="data-source-card" style="margin-top:12px"><span class="badge badge-net">נתונים ישנים · E3</span><p class="tiny">לא נשמר localCluster לקטע זה בשכבה הישנה.</p></div>`;return `<div class="data-source-card" style="margin-top:12px"><span class="badge badge-net">נתונים ישנים · E3</span><h4>${useLegacy?'האשכולות ההיסטוריים, כולל שכבת השוליים':'האשכולות המקומיים המתוקנים'}</h4>${!useLegacy&&sec?.localClustersLegacy?'<div class="notice v9-correction tiny"><strong>⟦תיקון V9⟧</strong> האשכולות חושבו מחדש ללא שכבת השוליים כאילו הייתה עד נוסח עצמאי.</div>':''}${cls.map(c=>`<div class="old-cluster-row"><strong>${esc(c.id)}</strong><span>${(c.members||[]).map(x=>esc(displayName(x))).join(' · ')}</span><small>mean=${fmt(c.meanSimilarity)} · threshold=${fmt(c.threshold)}</small></div>`).join('')}</div>`}
  function renderSectionClusterOverview(){const rows=(OLD.index.sections||[]).map(x=>String(x.id)).map(id=>{const sec=window.RAMBAN_V2_SECTIONS[id],defaultIds=sec?.v9DefaultCommentators||sec?.similarityMatrix?.defaultCommentators,members=(defaultIds?.length??oldSectionSummary[id]?.metrics?.nMembers??sec?.similarityMatrix?.commentators?.filter(x=>!isMarginaliaId(x)).length??0),cls=sec?.localClusters||[],assigned=new Set(cls.flatMap(c=>c.members||[]).filter(x=>!isMarginaliaId(x))),single=Math.max(0,members-assigned.size),sizes=cls.map(c=>(c.members||[]).filter(x=>!isMarginaliaId(x)).length).filter(Boolean);if(single)sizes.push(...Array(single).fill(1));return {id,members,sizes,clusters:cls.length,flag:oldSectionSummary[id]?.metrics?.flag||''}});return `<div class="section-cluster-overview">${rows.map(r=>`<button class="section-cluster-row ${String(state.section)===r.id?'is-active':''}" data-set-section="${escAttr(r.id)}"><span class="section-cluster-label">§${esc(r.id)}<small>${esc(sectionMap[r.id]?.passage_suggested||'')}</small></span><span class="cluster-strip">${r.sizes.length?r.sizes.map((z,i)=>`<i class="cluster-segment ${z===1?'single':''}" style="flex:${z}" title="${z===1?'יחיד':`קבוצה: ${z}`} מפרשים"></i>`).join(''):'<i class="cluster-segment single" style="flex:1"></i>'}</span><span class="section-cluster-meta">${r.clusters} קבוצות${r.flag?` · ${esc(r.flag)}`:''}</span></button>`).join('')}</div>`}
  ...
  function renderAnomalyHeatmap(container){if(!container)return;const ids=['DaatChacham_Vat107_Main','DaatChacham_Vat214_Main','DaatChacham_Vat214_Marginalia','UnKnownEskorial'];renderHeatmap(container,ids,ids,(a,b)=>a===b?1:num(pairMeans.get(pairKey(a,b))?.mean_raw),{min:0,max:1,rowName:displayName,colName:displayName,showValues:true,title:(a,b,v)=>`${displayName(a)} ↔ ${displayName(b)} · mean raw ישיר ${fmt(v)}`,onClick:(a,b)=>{if(a===b)return;state.a=a;state.b=b;syncSelectors();renderClusters()}})}
  ...
  function renderDispersionSummary(){const rows=analysisDecompRows(),l=rows.map(r=>num(r.locus_overlap)).filter(Number.isFinite),rd=rows.map(r=>num(r.reading_agreement)).filter(Number.isFinite),sharedNoComp=rows.filter(r=>num(r.n_shared_concepts)>0&&num(r.n_comparable_claims)===0).length,comparable=rows.filter(r=>num(r.n_comparable_claims)>0),zeroAgree=comparable.filter(r=>num(r.reading_agreement)===0).length;return `<div class="grid-4">${qCard('חציון חפיפת מוקדים',median(l),'האם עוסקים באותו חומר','נתונים חדשים · כימות')}${qCard('חציון הסכמת קריאה',median(rd),'האם אומרים אותו דבר','נתונים חדשים · כימות')}${countCard('משותף, בלי טענה בת־השוואה',sharedNoComp,`${(100*sharedNoComp/rows.length).toFixed(1)}% מתוך ${rows.length.toLocaleString('he-IL')}`,'נתונים חדשים · כימות')}${countCard('אפס הסכמה כשיש השוואה',zeroAgree,`${(100*zeroAgree/comparable.length).toFixed(1)}% מתוך ${comparable.length}`,'נתונים חדשים · כימות')}</div><div class="notice v9-correction" style="margin-top:12px"><strong>⟦תיקון V9⟧</strong> אלה חישובים ישירים מחדש לאחר הוצאת שכבת השוליים מאוכלוסיית ברירת־המחדל. שכבת השוליים עדיין זמינה במצב „כלול שכבות שוליים”.</div>`}
  function renderDecompFullTable(){const rows=analysisDecompRows();return `<div class="table-wrap" style="max-height:560px"><table class="data-table"><thead><tr><th>קטע</th><th>א׳</th><th>ב׳</th><th>חפיפת מוקדים</th><th>הסכמת קריאה</th><th>מושגים משותפים</th><th>טענות בנות־השוואה</th></tr></thead><tbody>${rows.map(r=>`<tr><td>§${esc(r.section)}</td><td>${esc(displayName(r.a))}</td><td>${esc(displayName(r.b))}</td><td class="num">${fmt(r.locus_overlap)}</td><td class="num">${fmt(r.reading_agreement)}</td><td class="num">${fmt(r.n_shared_concepts,0)}</td><td class="num">${fmt(r.n_comparable_claims,0)}</td></tr>`).join('')}</tbody></table></div>`}

  function renderRambanClosenessTable(){const rows=(Q['קרבה_לרמבן']||[]).slice().sort((a,b)=>(num(b.mean_rank)??-1)-(num(a.mean_rank)??-1));return `<div class="table-wrap" style="margin-top:14px"><table class="data-table"><thead><tr><th>דירוג V9</th><th>חיבור</th><th>סעיפים</th><th>D1 content</th><th>D5 order</th><th>co-cluster ⟦מושהה V9⟧</th><th>register</th></tr></thead><tbody>${rows.map((r,i)=>`<tr class="clickable" data-cluster-pair="1" data-a="${escAttr(r.commentator)}" data-b="RAMBAN"><td>${i+1}</td><td>${esc(displayName(r.commentator))}</td><td class="num">${fmt(r.sections,0)}</td><td class="num">${fmt(r.content_D1cov)}</td><td class="num">${fmt(r.order_D5)}</td><td class="num" title="⟦תיקון V9⟧ הערך ההיסטורי V8: ${fmt(r.legacy_co_cluster)}">—</td><td class="num">${fmt(r.register_sim)}</td></tr>`).join('')}</tbody></table><div class="notice v9-correction tiny"><strong>⟦תיקון V9⟧</strong> דירוג V9 מחושב משלושת הממדים הישירים: תוכן, סדר ורגיסטר. קו־האשכול ההיסטורי נשמר רק לצורכי audit ודורש הרצה מחדש של הפייפליין המקורי.</div></div>`}
  ...
  function renderTypeShareBars(container){if(!container)return;const rows=reportTypeShareRates(),W=700,H=300,p={l:190,r:36,t:18,b:25},barH=28,gap=8,max=.42;let out='';rows.forEach((r,i)=>{const y=p.t+i*(barH+gap),w=(W-p.l-p.r)*r.value/max;out+=`<g data-share-type="${escAttr(r.type)}"><text x="${p.l-10}" y="${y+19}" text-anchor="end" class="bar-label">${esc(r.label)}</text><rect x="${p.l}" y="${y}" width="${w}" height="${barH}" rx="5" class="type-share-bar"><title>${esc(r.label)} · ערך V8 היסטורי ${r.value.toFixed(3)} (${(r.value*100).toFixed(1)}%) · ⟦תיקון V9⟧ דורש rerun למדד הקורפוסי</title></rect><text x="${p.l+w+7}" y="${y+19}" class="bar-value">${r.value.toFixed(3)}*</text></g>`});container.innerHTML=`<div class="notice v9-correction tiny"><strong>⟦תיקון V9⟧</strong> העמודות להלן הן ערכי V8 היסטוריים בלבד. שיעור שיתוף לפי סוג מושג הוא מדד תלוי־אוכלוסייה ודורש הרצת הפייפליין מחדש. מפת פרשן×סוג קודקוד שמתחתיה מחושבת ישירות ותקפה כברירת־מחדל ללא marginalia.</div><svg viewBox="0 0 ${W} ${H}" class="bar-svg">${out}</svg>`;container.querySelectorAll('[data-share-type]').forEach(g=>g.addEventListener('click',()=>{state.textTagMode='custom';state.textActiveTypes=[g.dataset.shareType];switchTab('journey')}))}
  ...
  async function renderDispersionLab(){
    ...
    root.innerHTML=`
      <section class="panel dispersion-hero">
        <div class="panel-head"><div><div class="eyebrow">V9 · Interactive dispersion laboratory</div><h2>מעבדת פיזור: מוקד, קריאה והקשר</h2><p>זוגות ברירת־המחדל המתוקנים הופכים כאן למרחב חקר חי. כל נקודה היא זוג פרשנים בתוך קטע. הסרגל הקבוע שלמטה מסנן את <strong>כל התצוגות בעמוד</strong>, כך שאפשר להשוות קטע, פרשן, אשכול או קבוצת פרשנים בלי לאבד את ההקשר.</p></div><button class="small-btn" id="dispBackClusters">חזרה לאשכולות ופיזור</button></div>
      </section>
      <section class="dispersion-sticky-controls" id="dispersionStickyControls">
        ...
        <div class="dispersion-controls dispersion-controls-v6">
          ...
          <label><span>שכבות שוליים</span><label class="small-btn"><input id="dispIncludeMarginalia" type="checkbox" ${state.includeMarginalia?'checked':''}> כלול marginalia</label></label>
    ...
    root.querySelector('#dispIncludeMarginalia')?.addEventListener('change',e=>{state.includeMarginalia=e.target.checked;renderDispersionLab()});
  ...
  function dispersionFilteredRows(){
    let rows=(state.includeMarginalia?(Q['פירוק_התפזרות']||[]):defaultDecompRows()).map((r,i)=>{const p=getPairRow(r.section,r.a,r.b)||{};return {...r,_index:i,raw:num(p.raw),bits:num(p.bits),within_section_z:num(p.within_section_z)}});
  ...
  function renderDispersionLabViews(root){
    const rows=dispersionFilteredRows(),base=state.includeMarginalia?(Q['פירוק_התפזרות']||[]):defaultDecompRows();
    const medL=median(base.map(r=>num(r.locus_overlap)).filter(Number.isFinite))||0,medR=median(base.map(r=>num(r.reading_agreement)).filter(Number.isFinite))||0;
    renderDispersionLiveScatter(root.querySelector('#dispLiveScatter'),rows,medL,medR,root);
    root.querySelector('#dispCount').textContent=`${rows.length.toLocaleString('he-IL')} מתוך ${base.length.toLocaleString('he-IL')} נקודות`;
    ...
    root.innerHTML=`
      <section class="panel"><div class="panel-head"><div><h2>מפת מקורות הנתונים</h2><p>הפרדה בין שכבת המקור הפרשנית לבין שכבות המדידה — בלי לאבד את הקישור ביניהן.</p></div></div>
        <div class="grid-3">
          ...
          <div class="data-source-card"><span class="badge badge-quant">נתונים חדשים · כימות</span><h4>זוגות, פירוק התפזרות, אשכולות וראיות</h4><p class="tiny muted">${fmt(defaultRankedRows().length,0)} זוגות ברירת־מחדל (${fmt((Q['כל_הזוגות_מדורגים']||[]).length,0)} כולל marginalia) · ${fmt(defaultDecompRows().length,0)} רשומות פירוק ברירת־מחדל · ${fmt((Q['מושגים_משותפים_לרבים']||[]).length,0)} מושגים.</p></div>
        </div>
      </section>
  ...
  function globalPairValue(a,b,metric){
    if(canon(a)===canon(b))return 1;
    if(metric==='co_cluster'){const r=coClusterMap.get(pairKey(a,b));return r?.v9_status==='recomputed'?num(r.co_cluster_rate):null;}
  ...
  function globalMetricOptions(sel){return [['co_cluster','קו־אשכול · V8 היסטורי / מושהה ב־V9'],['mean_raw','ממוצע raw · נתונים חדשים'],['mean_bits','ממוצע bits · נתונים חדשים'],['mean_locus','ממוצע חפיפת מוקדים · נתונים חדשים'],['mean_reading','ממוצע הסכמת קריאה · נתונים חדשים'],['signature','מטריצת חתימה · נתונים ישנים']].map(([v,l])=>`<option value="${v}" ${v===sel?'selected':''} ${v==='co_cluster'?'disabled':''}>${l}</option>`).join('')}
  ...
  const TERM_GUIDES = {
    ...
    'קו־אשכול': 'מתוך כל הקטעים שבהם שני חיבורים מופיעים יחד, החלק שבו הם נופלים באותה קבוצה. 1.00 = תמיד יחד. ⟦תיקון V9⟧ ערכי V8 הושפעו מכך שהערות השוליים בוותיקן 214 נספרו בטעות כעד עצמאי. ב־V9 כל ערכי קו־האשכול של הפייפליין החדש מושהים עד הרצת הפייפליין המקורי ללא שכבת השוליים. החציון ≈0.32 הוא נתון היסטורי של V8 ואינו מוצג כממצא V9 מתוקן.',
    'co-cluster': 'השם האנגלי של קו־אשכול: באיזה חלק מן הקטעים המשותפים שני הפרשנים נמצאים באותה קבוצה. ⟦תיקון V9⟧ בחלק מן הזוגות המדד מושהה עד חישוב מחדש משום שהוא תלוי בהרכב אוכלוסיית הפרשנים.',
    ...
    'אחוזון': 'מיקום של ערך בתוך ההתפלגות. למשל אחוזון 97 פירושו שכ־97% מן הערכים נמוכים ממנו. ב־V9 raw=0.50 נמצא בערך באחוזון 97.0 באוכלוסיית ברירת־המחדל.',
  ...
  const INFO_GUIDES = [
    ...
    ['כתב יד אחד, שתי שכבות', '⟦תיקון V9⟧ מפת חום ממוקדת בדעת החכם ותיקן 107, הטקסט הראשי של ותיקן 214, הערות השוליים בוותיקן 214 והפירוש הלא־נודע מאסקוריאל. התאים משתמשים בדמיון תוכן ממוצע ישיר (mean raw), כדי להראות את קרבת שני עדי הנוסח מצד אחד ואת התנהגות שכבת השוליים מצד אחר, בלי להסתמך על קו־אשכול שדורש הרצה מחדש.'],
  ...
  function setupInfoSystem(){
    ...
    document.getElementById('openV9Correction')?.addEventListener('click',()=>{state.reportId='v9:CORRECTION_PROTOCOL';switchTab('reports')});
  ...
  function contextualInfoExtra(title){
    ...
    if(title.includes('כימות של היחסים')){const p=getPairRow(state.section,state.a,state.b),d=getDecompRow(state.section,state.a,state.b),cc=coClusterMap.get(pairKey(state.a,state.b));return ` במופע הנוכחי: §${state.section}, ${pair}. raw=${fmt(p?.raw)}, bits=${fmt(p?.bits)}, חפיפת מוקדים=${fmt(d?.locus_overlap)}, הסכמת קריאה=${fmt(d?.reading_agreement)}, קו־אשכול=${Number.isFinite(num(cc?.co_cluster_rate))?fmt(cc.co_cluster_rate):'מושהה ב־V9'}.`}
    ...
    if(title.includes('אופי ההתפזרות: מוקד לעומת קריאה')||title.includes('מעבדת פיזור')){const rows=state.tab==='dispersion'?dispersionFilteredRows():analysisDecompRows();return ` במופע הנוכחי מוצגות ${rows.length.toLocaleString('he-IL')} נקודות${state.tab==='dispersion'?' לאחר המסננים הפעילים':''}.`}
